<?php 
function stopTime($user_name){
    $loan_model = M('Loan');
    $map['user_name'] = $user_name;
    $info = $loan_model->where($map)->find();
    if ($info['renewal_days']) {
        if($info['loan_time']==1){
           $loan_time=7;
        }else if($info['loan_time']==2){
            $loan_time=14;
        }
        $stop = $info['is_pay']+$loan_time*86400+$info['renewal_days']*86400;
        $stop =date('Y-m-d H:i',$stop);
       return $stop;
    }
}
function amount1($n){
...
function interest1($n){
	$money = $n*0.05;
	return $money;
}
function interest2($n){
	$money = $n*0.10;
	return $money;
}


function interests1($n){
	$money = $n*0.10;
	return $money;
}
function interests2($n){
	$money = $n*0.15;
	return $money;
}

function original1($n){
	$day = date('Y-m-d H:i',$n+7*24*60*60);
	return $day;
}

function original2($n){
	$day = date('Y-m-d H:i',$n+14*24*60*60);
	return $day;
}
// 续期7天
function renewal1($n){
...
// 续期14天
function renewal2($n){
	$day = date('Y-m-d H:i',$n+28*24*60*60);
	return $day;
}
function renewal3($n){
	$day = date('Y-m-d H:i',$n+21*24*60*60);
	return $day;
}
function res1($n){
	$day = date('Y-m-d H:i',$n+14*24*60*60);
...
function banHome($bank){
    ...
    $bankName = array(   
     ...
     "玉溪市商业银行·红塔卡" ,"平安银行","平安银行","平安银行","平安银行","深圳发展银行","深圳发展银行","深圳发展银行","深圳发展银行","深圳发展银行","深圳发展银行","深圳发展银行","深圳发展银行","深圳发展银行","深圳发展银行");  
                $charBin = substr($bank, 0,6); 
                                $bin = 0; $index = 0;    

                $index= array_search($charBin,$bankBin);    
                if ($index == -1 ||  $index> count($bankName) ){    
                    return null;    
                } 
