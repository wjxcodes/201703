        ...
        <div class="col-sm-12">
            <div class="ibox float-e-margins">
                <div class="ibox-title">
                    <h5>成员列表</h5><a href="addauth"><button>增加权限</button></a>
                    <a href="add"><button style="float:right">增加</button></a>
                </div>
                <div class="ibox-content">
                    ...
                    <table class="footable table table-stripped toggle-arrow-tiny" data-page-size="8">
                        ...
                        <tbody>
                        <?php if(is_array($auth_data)): foreach($auth_data as $key=>$v): ?><tr>
                                <td><?php echo ($v['authname']); ?></td>
                                <td><?php echo ($v['username']); ?></td>
                                <td><?php echo (date('Y-m-d H:i',$v['create_time'])); ?></td>
                                <td><?php echo (date('Y-m-d H:i',$v['update_time'])); ?></td>
                                <td>
                                <a href="<?php echo U('Home/Manager/update',array('id'=>$v['id']));?>"><button>修改</button></a> 
                                <a href="<?php echo U('Home/Manager/del',array('id'=>$v['id']));?>"><button>删除</button></a>
                                </td>
                            </tr>
                            <tr>
                                <td>权限</td>
                                <td colspan="4">
                                    <?php if(is_array($auth_limit_data)): $i = 0; $__LIST__ = $auth_limit_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if(in_array( $vo['controller'],$v['new_auth'])){echo $vo['module'].'-';} endforeach; endif; else: echo "" ;endif; ?>
                                </td>
